...
class WatchdogThread(QThread):
        ...
        try:
            ...
            class SystemWatchdog:  # type: ignore
                ...
                def perform_emergency_shutdown(self) -> None:
                    """No emergency shutdown in stub implementation; intentionally left empty."""
    ...
    def _collect_cpu_with_fallback(self) -> float:
        ...
        try:
            # Local import for performance and to avoid side-effects at module import time.
            import psutil
    ...
    def _collect_processes_with_fallback(self) -> tuple[int, int]:
        ...
        try:
            ...
            if callable(process_method):
                ...
                logger.warning(
                    "count_dinoair_processes method not found on watchdog instance; using fallback value."
                )
                dinoair_count = self.fallback.dinoair_processes
    ...
    def _update_component_health(
        self, component: str, status: ComponentHealth, message: str
    ) -> None:
    ...
    def _check_thresholds(self, metrics: SystemMetricsT) -> None:
